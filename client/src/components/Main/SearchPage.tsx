...
function SearchPage() {
  ...
  const [searchResults, setSearchResults] =
    useRecoilState<SearchResults>(searchResultsAtom);
  ...
  async function fetchAlbums() {
    let data = await axios.get(`${searchUrl}/albums/${searchText}`, {
      headers: {
  ...
  async function fetchPlaylist() {
    let data = await axios.get(`${searchUrl}/playlist/${searchText}`, {
      headers: {
        Authorization: localStorage.getItem("token"),
      },
    });
  ...
  async function fetchArtists() {
    let data = await axios.get(`${searchUrl}/artists/${searchText}`, {
      headers: {
        Authorization: localStorage.getItem("token"),
      },
    });
  ...
  return (
    <>
      <div className="grid grid-cols-3 gap-2">
        {searchResults && searchResults.albums.length > 0 && (
          <div className="col-span-1 ">
            <TopResultCard album={searchResults && searchResults.albums[0]} />
          </div>
        )}
      ...
      <DisplayAlbums searchResults={searchResults} />
      <DisplayPlaylists searchResults={searchResults} />
...
function DisplayPlaylists({ searchResults }: { searchResults: SearchResults }) {
  return (
    <>
            ...
            searchResults.playlists.map((playlist: Playlist) => {
              return (
                <PlaylistCard
                  playlist={playlist}
                  key={playlist.id}
                  path={"/home/playlist/" + playlist.id}
                />
              );
            })}
...
function DisplayAlbums({ searchResults }: { searchResults: SearchResults }) {
  return (
    <>
      {/* Albums  */}
